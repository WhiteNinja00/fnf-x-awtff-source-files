...
class OutdatedSubState extends MusicBeatState
{
	...
	public static var needVer:String = "ERROR";
	public static var currChanges:String = "ERROR";
	...
	{
		super.create();
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('background', 'shared'));
		bg.scale.x *= 1.55;
		bg.scale.y *= 1.55;
		bg.screenCenter();
		add(bg);
		...
		var txt:FlxText = new FlxText(0, 0, FlxG.width,
			"oh no the kade is outdated!\nyou are on "
			+ MainMenuState.kadeEngineVer
			+ "\nwhen the most recent version is " + needVer + "."
			+ "\n\nthis is the new stuff:\n\n"
			+ currChanges
			+ "\nwhite ninja is gonna be pissed lmao"
			+ "\n\npress space to view the new kade\nor esc to play",
			32);
